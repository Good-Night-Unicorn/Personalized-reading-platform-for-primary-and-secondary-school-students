	...
	<view class="content">
		<view :style='{"alignContent":"flex-start","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
			<view class="list-swiper-4" :style='{"width":"calc(100% - 48rpx)","margin":"20rpx auto","position":"relative","height":"360rpx"}' @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd">
				<view :style='{"top":"0%","borderRadius":"32rpx","left":"0%","background":"none","width":"100%","position":"absolute","height":"360rpx"}' class="item animate__animated" :class="prevNumList4 == index  ? 'animate__zoomOut' : (numList4 == index  ? 'animate__zoomIn' : '')" v-for="(swiper,index) in swiperList" :key="index" v-if="numList4 == index || prevNumList4 == index">
			...
			<!-- 商品推荐 -->
			<view class="listBox recommend" :style='{"width":"calc(100% - 48rpx)","padding":"0 24rpx","margin":"40rpx auto","background":"none","order":"1"}'>
				<view class="title" :style='{"padding":"0 0 12rpx","margin":"0 0 40rpx","borderColor":"#666","textAlign":"center","background":"none","borderWidth":"0 0 2rpx","width":"100%","position":"relative","borderStyle":"solid","fontWeight":"600"}'>
					<view :style='{"color":"#000","fontSize":"36rpx"}'>书籍信息推荐</view>
				</view>
				<!-- 样式1 -->
				<view class="list-box style1" :style='{"width":"100%","fontSize":"26rpx","columnCount":"2","columnGap":"30rpx","color":"#888","height":"auto"}'>
					<view @tap="onDetailTap('shujixinxi',product.id)" v-for="(product,index) in shujixinxilist" :key="index" class="list-item" :style='{"border":"2rpx solid #ccc","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","height":"auto"}'>
						<view :style='{"padding":"0 0px","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.shujimingcheng}}</view>
						<view :style='{"padding":"0 0px","overflow":"hidden","whiteSpace":"nowrap","color":"#000","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}' class="list-item-title ">{{product.shujifenlei}}</view>
						<image :style='{"padding":"0","margin":"0 0 6rpx","objectFit":"cover","borderRadius":"0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-if="product.tupian.substring(0,4)=='http'" :src="product.tupian"></image>
						<image :style='{"padding":"0","margin":"0 0 6rpx","objectFit":"cover","borderRadius":"0","display":"block","width":"100%","height":"240rpx","order":"-1"}' class="list-item-image" mode="aspectFill" v-else :src="product.tupian?baseUrl+product.tupian.split(',')[0]:''"></image>
						<view :style='{"width":"100%","padding":"0 10rpx 0 0"}'>
						...
						<view :style='{"padding":"0 10rpx 0 0","display":"inline-block"}'>
							<text class="icon iconfont icon-geren16" :style='{"margin":"0 0px 0 0","lineHeight":"1.5","fontSize":"inherit","color":"inherit"}'></text>
							<text :style='{"color":"inherit","lineHeight":"1.5","fontSize":"inherit"}'>{{product.zuozhezhanghao}}</text>
						</view>
						<view :style='{"padding":"0 10rpx 0 0","display":"inline-block"}'>
							<text class="icon iconfont icon-shoucang10" :style='{"margin":"0 0px 0 0","lineHeight":"1.5","fontSize":"inherit","color":"inherit"}'></text>
			...
			<!-- 商品列表 -->
			<!-- 新闻资讯 -->
			<view class="listBox news" :style='{"padding":"0 24rpx","margin":"40rpx auto","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 48rpx)","position":"relative","height":"auto","order":"2"}'>
				<view class="title" :style='{"padding":"0 0 12rpx","margin":"0 0 40rpx","borderColor":"#666","textAlign":"left","background":"none","borderWidth":"0 0 2rpx","width":"100%","position":"relative","borderStyle":"solid","fontWeight":"600"}'>
					<view :style='{"color":"#000","fontSize":"36rpx"}'>公告资讯</view>
					<view :style='{"padding":"0","alignItems":"center","top":"0px","borderRadius":"12rpx","background":"none","display":"flex","lineHeight":"48rpx","position":"absolute","right":"0px","justifyContent":"center","zIndex":"999"}' @tap="onPageTap('news')">
					  <text :style='{"color":"#999","fontSize":"28rpx","fontWeight":"500"}'>更多</text>
					  <text class="icon iconfont icon-jinru" :style='{"color":"#999","fontSize":"28rpx","fontWeight":"500"}'></text>
					</view>
				</view>
				<!-- 样式9 -->
				<view class="news-box6" :style='{"width":"100%","padding":"0","margin":"0","fontSize":"26rpx","color":"#888","height":"auto"}'>
					<view @tap="onNewsDetailTap(item.id)" v-for="(item,index) in news" :key="index" class="list-item" :style='{"border":"2rpx solid #ccc","boxShadow":"none","padding":"0","margin":"0 0 40rpx","backgroundColor":"#fff","borderRadius":"8rpx","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","height":"auto"}'>
						<view :style='{"padding":"0","margin":"0 0 0 0","color":"#333","textAlign":"center","right":"20rpx","borderRadius":"200rpx","top":"20rpx","background":" #eee","width":"52rpx","lineHeight":"52rpx","fontSize":"28rpx","position":"absolute","height":"52rpx"}' class="num">
						...
						<view class="item-list-body" :style='{"width":"100%","padding":"20rpx 20rpx","margin":"0","borderRadius":"0 60rpx  0 0","height":"auto"}'>
							<view :style='{"padding":"4rpx 0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"88%","lineHeight":"60rpx","fontSize":"28rpx","textOverflow":"ellipsis","fontWeight":"600"}' class="title ">{{item.title}}</view>
							...
							<view :style='{"width":"100%","margin":"0 20rpx 0 0","display":"inline-block"}'>
								<text class="icon iconfont icon-shijian21" :style='{"margin":"0 4rpx 0 0","lineHeight":"1.5","fontSize":"inherit","color":"inherit"}'></text>
								<text :style='{"color":"inherit","lineHeight":"1.5","fontSize":"inherit"}'>{{item.addtime.split(' ')[0].replace(/\-/g,'-')}}</text>
							</view>
							<view :style='{"margin":"0 20rpx 0 0","display":"inline-block"}'>
	...
	export default {
		...
		data() {
			return {
				...
				user: {},
				tableName:'',
				menuColor: '#000,#666,#888,#999,#000,#555,#666,#888,#999'.split(','),
		...
		methods: {
			...
			async getRecommendList(){
				...
				if(uni.getStorageSync("appUserid")) {
					res = await this.$api.recommend2('shujixinxi', params);
				} else {
					res = await this.$api.recommend('shujixinxi', params);
				}
