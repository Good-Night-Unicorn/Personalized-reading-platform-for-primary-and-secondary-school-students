	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zuozhe`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.zuozhezhanghao)){
					this.$utils.msg(`作者账号不能为空`);
					return
				}
				...
				if((!this.ruleForm.zuozhexingming)){
					this.$utils.msg(`作者姓名不能为空`);
					return
				}
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`zuozhe`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`zuozhe`, this.ruleForm);
						}else{
							oet = await this.$api.add(`zuozhe`, this.ruleForm);
						}
						if(this.cross){
