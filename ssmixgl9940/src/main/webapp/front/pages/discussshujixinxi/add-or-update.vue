	...
	<view :style='{"width":"100%","padding":"40rpx 0 0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"0 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"10rpx 20rpx","margin":"0","borderColor":"#ccc","borderRadius":"0","borderWidth":"0 0 0px 0","background":"#fff","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<xia-editor ref="content" :style='{"minHeight":"300rpx","border":"2rpx solid #666","width":"100%","padding":"20rpx","borderRadius":"8rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
			<view :style='{"padding":"10rpx 20rpx","margin":"0","borderColor":"#ccc","borderRadius":"0","borderWidth":"0 0 0px 0","background":"#fff","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","fontWeight":"600"}' class="title">评分</view>
			...
			async onSubmitTap() {
				...
				//跨表计算
						...
						if(this.ruleForm.id){
							...
							oet = await this.$api.add(`discussshujixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussshujixinxiCleanType',true);
						let rs2 = await this.$api.list('discussshujixinxi',{page: 1,limit: 10000,refid: this.ruleForm.refid})
						let totalscore = 0
						for(let x in rs2.data.list){
							totalscore += Number(rs2.data.list[x].score)
						}
						let rs3 = await this.$api.info('shujixinxi',this.ruleForm.refid)
						rs3.data.totalscore = Number((totalscore / rs2.data.list.length).toFixed(2))
						await this.$api.update('shujixinxi',rs3.data)
						if(this.cross){
